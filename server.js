...
const ADMIN_PASSWORD = normalizeSecret(process.env.ADMIN_PASSWORD);
...
function isAdmin(req){const token=parseCookies(req).ntg_session; return token && sessions.has(token)}
...
const server=http.createServer(async(req,res)=>{
 try{
  ...
  if(req.method==='POST' && u.pathname==='/api/admin/login'){
   const x=await body(req); const supplied=Buffer.from(normalizeSecret(x.password)); const expected=Buffer.from(ADMIN_PASSWORD); if(!expected.length || !supplied.length || supplied.length!==expected.length || !crypto.timingSafeEqual(supplied,expected)) return json(res,401,{error:'পাসওয়ার্ড সঠিক নয়।'});
   const token=crypto.randomBytes(32).toString('hex');sessions.set(token,Date.now()+8*60*60*1000);setTimeout(()=>sessions.delete(token),8*60*60*1000);
   res.writeHead(200,{'Content-Type':'application/json','Set-Cookie':`ntg_session=${token}; HttpOnly; SameSite=Lax; Path=/; Max-Age=28800${process.env.NODE_ENV==='production'?'; Secure':''}`});return res.end(JSON.stringify({ok:true}));
  }
